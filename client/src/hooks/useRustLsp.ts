...
export interface RustHoverResult {
  contents: string;
}
...
// Hierarchical DocumentSymbol (LSP spec). rust-analyzer returns this shape when
// the client advertises hierarchicalDocumentSymbolSupport. Identical to the
// cpp/ocaml shapes so OutlinePanel renders all three the same way.
export interface RustDocumentSymbol {
  name: string;
  detail?: string;
  kind: number;
  range: LspRange;
...
export function useRustLsp(
  ...
  // The server-side idle timer then reaps rust-analyzer; reconnect on focus.
  ...
  const connect = useCallback(() => {
    ...
    const url = `${protocol}//${host}/ws/rust/${sessionId}`;
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({
        ...
        params: {
          ...
          workspaceFolders: projectPath ? [{ uri: `file://${projectPath}`, name: 'pyramid-rust-session' }] : null,
          // rust-analyzer reads almost all of its config here. `check.command`
          // switches the on-save check between `cargo check` and clippy.
          initializationOptions: {
            check: { command: useClippy ? 'clippy' : 'check' },
            checkOnSave: true,
          },
          capabilities: {
            textDocument: {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // Server-initiated request (has id and method) — respond so rust-analyzer
        // doesn't hang. workspace/configuration wants an array (one entry per
        // requested item); everything else gets null. rust-analyzer falls back
        // to the initializationOptions for config, so null items are fine.
        if (msg.id !== undefined && msg.id !== null && msg.method) {
  ...
  }, [sessionId, active, projectPath, useClippy]);
  ...
  const requestCompletion = useCallback(async (uri: string, line: number, character: number) => {
    const result = await sendRequest<{ items?: RustCompletionItem[] } | RustCompletionItem[]>(
  ...
  const requestDocumentSymbols = useCallback(async (uri: string): Promise<RustDocumentSymbol[]> => {
    ...
    // rust-analyzer returns DocumentSymbol[] when hierarchical support is
    // advertised. Fall back to the flat SymbolInformation[] shape just in case.
    ...
    return result as RustDocumentSymbol[];
  }, [sendRequest]);
