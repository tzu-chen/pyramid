...
export interface OcamlCompletionItem {
  label: string;
  kind?: number;
  detail?: string;
...
export interface OcamlHoverResult {
  contents: string;
}
...
export interface OcamlDocumentSymbol {
  name: string;
  detail?: string;
  kind: number;
  range: LspRange;
  selectionRange: LspRange;
  children?: OcamlDocumentSymbol[];
}

export function useOcamlLsp(sessionId: string | undefined, enabled: boolean, projectPath: string | null = null) {
  ...
  const connect = useCallback(() => {
    ...
    const url = `${protocol}//${host}/ws/ocaml/${sessionId}`;
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({
        ...
        params: {
          ...
          workspaceFolders: projectPath ? [{ uri: `file://${projectPath}`, name: 'pyramid-ocaml-session' }] : null,
          capabilities: {
            textDocument: {
  ...
  // ocamllsp distinguishes .ml from .mli via languageId. Default to 'ocaml'
  // (implementation); the caller passes a URI from which we infer .mli.
  const languageIdFor = (uri: string): string =>
    uri.endsWith('.mli') ? 'ocaml.interface' : 'ocaml';
  ...
  const requestCompletion = useCallback(async (uri: string, line: number, character: number) => {
    const result = await sendRequest<{ items?: OcamlCompletionItem[] } | OcamlCompletionItem[]>(
  ...
  const requestDocumentSymbols = useCallback(async (uri: string): Promise<OcamlDocumentSymbol[]> => {
    ...
    return result as OcamlDocumentSymbol[];
  }, [sendRequest]);
